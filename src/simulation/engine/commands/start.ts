...
@Injectable()
export class Start implements SimulationCommandInterface
{
    constructor(
    ...
    public exec(state: SimulationStateInterface, command: SimulationCommandType, snapshots: SimulationSnapshotType[], registerWaitCommand: Boolean): SimulationCommandResType
    {
        ...
        {
            state.startAllStations();
        }
        else if (typeof command.stationId === 'number')
        {
            state.startStation(command.stationId);
        }
